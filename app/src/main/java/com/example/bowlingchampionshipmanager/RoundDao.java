...
public interface RoundDao {
    ...
    @Query("SELECT round.sys_roundID,round.round_uuid, round.froundid, round.fchampID, round.champ_uuid, round.team1ID, round.team2ID,round.team1UUID, round.team2UUID, round.score1, round.score2,round.status, round.date FROM round INNER JOIN team ON (round.team1UUID=team.team_uuid or round.team2UUID=team.team_uuid) WHERE (team.team_uuid=:teamuuid AND round.status='next' AND round.champ_uuid=:champuuid)")
    LiveData<List<Round>> getRoundsofTeam( String teamuuid,String champuuid); //todo na valw k order. To status den to 8elw mallon
