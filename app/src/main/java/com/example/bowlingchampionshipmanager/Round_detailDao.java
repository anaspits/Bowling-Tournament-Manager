...
public interface Round_detailDao {
    ...
    @Query("SELECT round_detail.* FROM round_detail INNER JOIN round ON round.round_uuid=round_detail.round_uuid WHERE round_detail.champ_uuid=:chid AND round.froundid<=:frid ORDER BY round.froundid") //todo gia gunaika kai antra?
    ...
    @Query("SELECT round_detail.round_uuid, round_detail.participant_uuid,round_detail.champ_uuid, round_detail.score, round_detail.avg, round_detail.hdcp, round_detail.first, round_detail.second, round_detail.third, round_detail.blind, round_detail.games, round_detail.froundid, round_detail.created_at, round_detail.updated_at FROM round_detail INNER JOIN round ON round.round_uuid=round_detail.round_uuid WHERE round_detail.participant_uuid=:pid AND round.champ_uuid=:chid AND round_detail.blind=0 AND round.status='done'")
    LiveData<List<Round_detail>> getAllDoneRound_detailofplayerofChamp(String pid, String chid);
    ...
    @Query("SELECT participant.participant_uuid,participant.first_name, participant.last_name,participant.sex,round_detail.round_uuid,round_detail.score,round_detail.avg,round_detail.hdcp ,round_detail.games, round_detail.froundid,round_detail.first, round_detail.second, round_detail.third,round_detail.updated_at FROM participant INNER JOIN round_detail ON participant.participant_uuid=round_detail.participant_uuid  WHERE round_detail.champ_uuid=:chid ORDER BY round_detail.froundid DESC")
    LiveData<List<PlayerandGames>> getAllPlayerScoreGamesofChampOrdered(String chid);

    @Query("SELECT participant.participant_uuid,participant.first_name, participant.last_name,participant.sex,round_detail.round_uuid,round_detail.score,round_detail.avg,round_detail.hdcp ,round_detail.games, round_detail.froundid,round_detail.first, round_detail.second, round_detail.third,round_detail.updated_at FROM participant INNER JOIN round_detail ON participant.participant_uuid=round_detail.participant_uuid INNER JOIN round ON round.round_uuid=round_detail.round_uuid WHERE round_detail.champ_uuid=:chid AND round_detail.blind=0 AND round.status='done' ORDER BY participant.last_name")
    LiveData<List<PlayerandGames>> getAllPlayerScoreGamesofChampofDoneRoundsOrderedByplayer(String chid);

    @Query("SELECT participant.participant_uuid,participant.first_name, participant.last_name,participant.sex,round_detail.round_uuid,round_detail.score,round_detail.avg,round_detail.hdcp ,round_detail.games, round_detail.froundid,round_detail.first, round_detail.second, round_detail.third,round_detail.updated_at FROM participant INNER JOIN round_detail ON participant.participant_uuid=round_detail.participant_uuid INNER JOIN team_detail ON team_detail.participant_uuid=participant.participant_uuid WHERE round_detail.champ_uuid=:chid AND team_detail.team_uuid=:tid ORDER BY round_detail.froundid DESC")
    LiveData<List<PlayerandGames>> getAllPlayerScoreGamesofTeamOrdered(String chid, String tid);

    @Query("SELECT participant.participant_uuid,participant.first_name, participant.last_name,participant.sex,round_detail.round_uuid,round_detail.score,round_detail.avg,round_detail.hdcp ,round_detail.games, round_detail.froundid,round_detail.first, round_detail.second, round_detail.third,round_detail.updated_at FROM participant INNER JOIN round_detail ON participant.participant_uuid=round_detail.participant_uuid INNER JOIN round ON round.round_uuid=round_detail.round_uuid INNER JOIN team_detail ON team_detail.participant_uuid=participant.participant_uuid WHERE round_detail.champ_uuid=:chid AND team_detail.team_uuid=:tid AND round.status='done' ORDER BY round_detail.froundid DESC")
