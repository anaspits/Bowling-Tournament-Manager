...
public interface Round_detailDao {
    ...
    @Query("SELECT participant.participant_uuid,participant.first_name, participant.last_name,round_detail.round_uuid,round_detail.avg,round_detail.hdcp ,round_detail.games FROM participant INNER JOIN round_detail ON participant.participant_uuid=round_detail.participant_uuid WHERE round_detail.champ_uuid=:chid AND round_detail.round_uuid=:rid ORDER BY round_detail.avg")
